...
export class GurpsCharacterModel {
    ...
    get totalPoints(): number {
        ...
        return (
            Object.values(this.basicTraits).reduce((previous: number, current: ITrait) => previous + current.points, 0) +
            Object.values(this.skills).reduce((previous: number, current: ITrait) => previous + current.points, 0) +
            Object.values(this.extraPositiveTraits).reduce((previous: number, current: ITrait) => previous + current.points, 0) +
            Object.values(this.extraNegativeTraits).reduce((previous: number, current: ITrait) => previous + current.points, 0)
        );
