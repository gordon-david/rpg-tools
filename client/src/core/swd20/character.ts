export class Ability {
  ...
  get finalScore() {
    return (
      this.baseValue +
      this.bonuses.reduce((p: number, c: AbilityBonus) => p + c.value, 0)
    );
  }
  serialize(): any {
    return {
...
export class AbilityBonus {
  ...
  constructor({
    value = 0,
    abilityName = null,
  }: { value?: number; abilityName?: AbilityName | null } = {}) {
  ...
  _addAbilityBonusesFromSource(source: Species) {
    for (const abilityName in this.abilities) {
      this.abilities[abilityName].bonuses = [
        ...this.abilities[abilityName].bonuses,
        ...source.abilityBonuses.filter((b) => b.abilityName === abilityName),
      ];
    }
  }
