...
export class SwadeSkill {
    ...
    constructor({ name = '', isCore = false, points = 0, derivedAttribute = new SwadeAttribute() } = {}) {
        this.name = name
        this._points = points
        this.isCore = isCore
    ...
    incrementValue() {
        const pointsDelta = this.points - (Math.floor(this.derivedAttribute.points / 2) + 1)
        if (pointsDelta < 0) {
            this.points += 1
    ...
    get value() {
        ...
        const pointsDelta = tempPoints - (Math.floor(this.derivedAttribute.points / 2) + 1)
        ...
        const effectivePoints = pointsBelowAttributeLevel + Math.floor(pointsAboveAttributeLevel / 2)
    ...
    constructor(oldState: any = getInitialState()) {
        ...
        for (let skillKey of Object.keys(oldState.skills)) {
            this.skills[skillKey] = new SwadeSkill({
                ...
                derivedAttribute: this.attributes[oldState.skills[skillKey].derivedAttribute]
            })
        }
    }
