...
export class GurpsBasicAttribute implements ITrait {
    ...
    displayName: string;
    abbreviation: string;
    constructor(key = "", { pointsPerLevel = 10, displayName = key, abbreviation = key } = {}) {
        this.key = key;
...
export class GurpsSkill implements ITrait {
    ...
    constructor(key: string, derivedAttribute: ITrait, skillDifficulty: SkillDifficulty) {
        this.key = key;
    ...
    get value() {
        ...
        return this.derivedAttribute.value + this.modifiers + skillInitialMod[this.skillDifficulty]
    }
...
export class DerivedCharacteristic implements ITrait {
    ...
    constructor(key: string, derivedAttribute: GurpsBasicAttribute, { pointsPerLevel = 10, displayName = key, abbreviation = key } = {}) {
        this.key = key;
        this.displayName = displayName
        this.abbreviation = abbreviation
        this.pointsPerLevel = pointsPerLevel;
...
class BasicSpeed extends DerivedCharacteristic {
    ...
    constructor(derivedHealth: GurpsBasicAttribute, derivedDexterity: GurpsBasicAttribute) {
        super('basicSpeed', derivedDexterity, { pointsPerLevel: 5, displayName: "Basic Speed", abbreviation: "BS" })
        this.derivedDexterity = derivedDexterity
        this.derivedHealth = derivedHealth
    }

    get value() {
        return ((this.derivedDexterity.value + this.derivedHealth.value) / 4) + (this.modifiers / 4)
    }
...
export class GurpsCharacterModel {
    ...
    constructor() {
        ...
        let strength = new GurpsBasicAttribute("strength", { pointsPerLevel: 10, displayName: "Strength", abbreviation: "ST" })
        let dexterity = new GurpsBasicAttribute("dexterity", { pointsPerLevel: 20, displayName: "Dexterity", abbreviation: "DX" })
        let intelligence = new GurpsBasicAttribute("intelligence", { pointsPerLevel: 20, displayName: "Intelligence", abbreviation: "IQ" })
        let health = new GurpsBasicAttribute("health", { pointsPerLevel: 10, displayName: "Health", abbreviation: "HT" })
        ...
        this.basicTraits.hitPoints = new DerivedCharacteristic('hitPoints', strength, { pointsPerLevel: 2, displayName: "Hit Points", abbreviation: "HP" })
        this.basicTraits.will = new DerivedCharacteristic('will', intelligence, { pointsPerLevel: 5, displayName: "Will", abbreviation: "Will" })
        this.basicTraits.fatiguePoints = new DerivedCharacteristic('fatiguePoints', health, { pointsPerLevel: 3, displayName: "Fatigue Points", abbreviation: "FP" })
        this.basicTraits.perception = new DerivedCharacteristic('perception', intelligence, { pointsPerLevel: 5, displayName: "Perception", abbreviation: "Per" })
        this.basicTraits.basicSpeed = new BasicSpeed(health, dexterity)
        this.basicTraits.basicMove = new BasicMove(this.basicTraits['basicSpeed'] as BasicSpeed)
    }
    ...
    get totalPoints(): number {
        ...
        return (
            ...
            Object.keys(this.extraPositiveTraits).reduce(
                (p, c) =>
                    ((p) => (typeof p === "number" ? p : this.extraPositiveTraits[p].points))(p) +
