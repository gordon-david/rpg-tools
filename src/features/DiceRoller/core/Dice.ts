...
class DiceFactory {
  static build(key: DiceKey): PolyehedralDice {
    switch (key) {
      case DiceKey.D4:
        return new PolyehedralDice("D4", key, 4);
      case DiceKey.D6:
        return new PolyehedralDice("D6", key, 6);
      case DiceKey.D8:
        return new PolyehedralDice("D8", key, 8);
      case DiceKey.D10:
        return new PolyehedralDice("D10", key, 10);
      case DiceKey.D12:
        return new PolyehedralDice("D12", key, 12);
      case DiceKey.D20:
        return new PolyehedralDice("D20", key, 20);
      case DiceKey.D100:
        return new PolyehedralDice("D100", key, 100);
      default:
        throw new Error("Dice type unsupported.");
    }
  }
}

export type DiceMap = { [DiceKey: string]: number };
...
class DicePool {
  ...
  clear() {
    Object.keys(this.diceMap).forEach((key) => (this.diceMap[key] = 0));
  }
  ...
  getDiceKeys(): DiceKey[] {
    return Object.keys(this.diceMap) as DiceKey[];
  }
  getDiceMap(): DiceMap {
    return this.diceMap;
  }
  getDice(): PolyehedralDice[] {
    return Object.values(this.diceCache);
  }
  count(diceKey: DiceKey): number {
    return this.diceMap[diceKey];
  }
