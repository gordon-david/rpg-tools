...
const addPolyhedralDice = (diceResultArray: any[]) => {
    return diceResultArray.reduce((p, c) => p + c, 0)
}

const addGenesysDice = (diceResultArray: any[]) => {
...
const Dice = (type: string, faces: string[][] | number[]) => {
    return {
...
const DiceFactory = (diceType: string) => {
    switch (diceType) {
 ...
 */
const DicePool = (diceTypeArray: string[]) => {
        ...
        roll() {
            ...
            Object.keys(currentSelection).forEach(diceType => {
                if (polyhedralTypes.map(value => value === diceType).length !== 0) {
                    ...
                    currentSelection[diceType].forEach((dice: any) => polyhedralPool.push(dice))
                }
                if (genesysTypes.map(value => value === diceType).length !== 0) {
                    currentSelection[diceType].forEach((dice: any) => genesysPool.push(dice))
                }
            })
            console.log(polyhedralTypes)
            const genesysResult = addGenesysDice(genesysPool.map((dice: any) => dice.roll()))
            const polyhedralResult = addPolyhedralDice(polyhedralPool.map((dice: any) => dice.roll()))
